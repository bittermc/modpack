...
__on_player_message(sender, content) -> ( // the main thing. looks really iffy
	...
	for (global_all_pingables,
		...
		if (pinged, for (pinged,
			delete(the_rest, the_reset~_);
			mimicry(_, sender, msg);
			run(str('/playsound %s player %s', global_all_configs:str(p):'pingSound', p));
		))
	);

	mimicry(the_rest, sender, content);
	//signal_event('player_message', player, sender, content);
